...
// ─── Tool registry ────────────────────────────────────────────────────────────
// abbr: text shown in the icon badge, color: brand background color

const TOOL_REGISTRY = {
  'blender':               { abbr: 'Bl',  color: '#EA7600' },
  'cinema 4d':             { abbr: 'C4D', color: '#0460D9' },
  'houdini':               { abbr: 'Hou', color: '#FF4713' },
  'after effects':         { abbr: 'Ae',  color: '#9999FF' },
  'photoshop':             { abbr: 'Ps',  color: '#31A8FF' },
  'zbrush':                { abbr: 'ZB',  color: '#5F3789' },
  'substance painter':     { abbr: 'SP',  color: '#E75B10' },
  'substance 3d painter':  { abbr: 'SP',  color: '#E75B10' },
  'unreal engine':         { abbr: 'UE',  color: '#1a1a1a' },
  'maya':                  { abbr: 'Ma',  color: '#37A5CC' },
  '3ds max':               { abbr: '3M',  color: '#87CFEE', textColor: '#0a0a0a' },
  'marvelous designer':    { abbr: 'MD',  color: '#2C9CB5' },
  'keyshot':               { abbr: 'KS',  color: '#E02222' },
  'nuke':                  { abbr: 'Nu',  color: '#F1B52D', textColor: '#0a0a0a' },
  'davinci resolve':       { abbr: 'DR',  color: '#BD6C28' },
  'premiere pro':          { abbr: 'Pr',  color: '#E040FB' },
  'illustrator':           { abbr: 'Ai',  color: '#FF9A00' },
  'redshift':              { abbr: 'RS',  color: '#C8121A' },
  'octane render':         { abbr: 'Oct', color: '#8DC8E8', textColor: '#0a0a0a' },
  'arnold':                { abbr: 'Ar',  color: '#222222' },
  'v-ray':                 { abbr: 'VR',  color: '#214499' },
  'figma':                 { abbr: 'Fi',  color: '#1A73E8' },
}

const PROCESS_TABS = [
  { key: 'preProduction',  label: 'Pre-Production' },
  { key: 'viewportView',   label: 'Viewport' },
  { key: 'renderedView',   label: 'Rendered' },
...
function ToolBadge({ name }) {
  ...
  const info = TOOL_REGISTRY[key]
  const abbr = info?.abbr ?? name.slice(0, 2).toUpperCase()
  const bg = info?.color ?? '#2a1f45'
  const fg = info?.textColor ?? '#ffffff'
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
      <div style={{
        ...
        flexShrink: 0,
        fontSize: abbr.length > 2 ? '0.46rem' : '0.58rem',
        fontFamily: "'Syne', sans-serif",
        fontWeight: 700,
        color: fg,
...
export default function ThreeDTemplate({ project }) {
  const [activeTab, setActiveTab] = useState('preProduction')
  const content = project.content ?? {}
  const finalMedia = content.final_media ?? {}
  const tools = content.tools ?? []
  const stats = content.stats ?? []
  const process = content.process_3d ?? {}

  const hasContent = (tab) => {
    const d = process[tab.key]
    return d?.modules?.length > 0
  }
